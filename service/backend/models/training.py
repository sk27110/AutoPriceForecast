...
def train_job(params: TrainParams, unique_model_id: str, model_type: ModelType) -> None:
    ...
    try:
        logger.info(
            "Начало обучения модели %s (%s)", unique_model_id, model_type.model_type
        )
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                logger.info("Модель %s обучена. Метрики: %s", unique_model_id, metrics)

            except FutureTimeoutError:
                logger.warning("Таймаут обучения модели %s", unique_model_id)

    except Exception as e:  # pylint: disable=broad-except
        logger.error(
            "Ошибка обучения модели %s: %s", unique_model_id, str(e), exc_info=True
        )
    finally:
        if pipeline is not None:
            MODELS[unique_model_id] = {"pipeline": pipeline, "metrics": metrics}
            try:
                joblib.dump(
                    MODELS[unique_model_id], MODEL_STORAGE / f"{unique_model_id}.pkl"
                )
                logger.info("Модель %s сохранена", unique_model_id)
            except Exception as e:  # pylint: disable=broad-except
                logger.error(
                    "Ошибка сохранения модели %s: %s",
                    unique_model_id,
                    str(e),
                    exc_info=True,
                )
